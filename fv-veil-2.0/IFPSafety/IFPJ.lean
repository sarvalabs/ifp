-- IFPJ: IFP model based on IFPI with height-aware ghost relations.
-- Adds locked_at_height, locked_descendant_at_height, proposed_at_height
-- to give SMT direct access to height information without existential chaining.
-- HotStuff uses height as a first-class ghost parameter (votedh, votedb, votedhh);
-- this brings IFP's approach closer to that pattern.

import Veil

veil module IFPProtocolJ
