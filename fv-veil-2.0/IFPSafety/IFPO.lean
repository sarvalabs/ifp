-- IFPO: IFP model with QC (Quorum Certificate) relations.
-- Key changes from IFPM:
-- 1. Added quorum_prevoted/quorum_precommitted relations (models prevoteQC/precommitQC)
-- 2. operator_prevote/operator_precommit set QC relations on quorum aggregation
-- 3. respond_prevote/respond_precommit reference QC relations instead of re-checking quorum existentials
-- 4. Quorum existentials isolated into linking invariants (qc_prevote_implies_quorum, etc.)
-- 5. Existing quorum-existential invariants replaced with QC-form equivalents

import Veil

veil module IFPProtocolO
...
-- QC relations (models prevoteQC / precommitQC artifacts)
-- No node parameter: QC is a protocol-level artifact, not per-node state
-- View + participants + interaction uniquely identifies the QC
relation quorum_prevoted : view → participant → participant → interaction → Bool
relation quorum_precommitted : view → participant → participant → interaction → Bool
...
-- Committed state (NEW in IFPM: tracks node's latest decided interaction)
-- committed_height is derived as height(committed_ixn) — no separate relation needed
...
after_init {
  ...
  -- QC init: no QCs exist at genesis
  quorum_prevoted V P Q I := false;
  quorum_precommitted V P Q I := false;
...
action propose_repropose (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ci : interaction) {
  require v ≠ tot_view.zero
  require (p1 = p1_fixed ∧ p2 = p2_fixed)
  require p1 ≠ p2
  require participant_context p1 c1 ∧ participant_context p2 c2
...
action propose_extend (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn_propose : interaction) (ci : interaction) {
  require v ≠ tot_view.zero
  require (p1 = p1_fixed ∧ p2 = p2_fixed)
  require p1 ≠ p2
  require participant_context p1 c1 ∧ participant_context p2 c2
  ...
  require committed_ixn op ci
  require height ixn_max_1 ≤ height ci
...
action propose_nil (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ci : interaction) {
  require v ≠ tot_view.zero
  require (p1 = p1_fixed ∧ p2 = p2_fixed)
  require p1 ≠ p2
  require participant_context p1 c1 ∧ participant_context p2 c2
  ...
  require committed_ixn op ci
  require (height ixn_max_1 > height ci + 1)
        ∨ (height ixn_max_1 = height ci + 1 ∧ s_max_1 = precommit)
...
action operator_prevote (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn : interaction) {
  ...
  quorum_prevoted v p1 p2 ixn := true
}
...
action respond_prevote (n : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn : interaction) {
  ...
  require quorum_prevoted v p1 p2 ixn
...
action operator_precommit (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn : interaction) {
  ...
  quorum_precommitted v p1 p2 ixn := true
}
...
action respond_precommit (n : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn : interaction) {
  ...
  require quorum_precommitted v p1 p2 ixn
...
-- Supporting: Prevote lock implies QC prevoted (replaces quorum-existential form)
invariant [prevote_lock_implies_qc_prevoted]
  (¬ ctx.is_byz N ∧ (locked N p1_fixed I prevote V ∨ locked N p2_fixed I prevote V) ∧ I ≠ genesis) →
    quorum_prevoted V p1_fixed p2_fixed I
...
-- Decided implies QC precommitted (replaces quorum-existential form)
invariant [decided_implies_qc_precommitted]
  (¬ ctx.is_byz N ∧ decided N V p1_fixed p2_fixed I ∧ I ≠ genesis) →
    quorum_precommitted V p1_fixed p2_fixed I
...
-- Operator prevoted implies QC prevoted (replaces quorum-existential form)
invariant [prevoted_operator_implies_qc]
  (¬ ctx.is_byz OP ∧ prevoted_operator OP V p1_fixed p2_fixed I) → quorum_prevoted V p1_fixed p2_fixed I

-- Operator precommitted implies QC precommitted (replaces quorum-existential form)
invariant [precommitted_operator_implies_qc]
  (¬ ctx.is_byz OP ∧ precommitted_operator OP V p1_fixed p2_fixed I) → quorum_precommitted V p1_fixed p2_fixed I
...
-- ####################################################################
-- # QC (Quorum Certificate) Invariants
-- ####################################################################

-- QC Linking: quorum_prevoted implies actual quorum witness exists
-- (Isolates the quorum existential — only operator_prevote sets this)
invariant [qc_prevote_implies_quorum]
  quorum_prevoted V p1_fixed p2_fixed I →
...
-- QC Linking: quorum_precommitted implies actual quorum witness exists
invariant [qc_precommit_implies_quorum]
  quorum_precommitted V p1_fixed p2_fixed I →
    (∃ (c1 c2 s1 s2 : nodeset), participant_context p1_fixed c1 ∧ participant_context p2_fixed c2 ∧
      interactions I p1_fixed p2_fixed ∧
      ctx.supermajority s1 c1 ∧ ctx.supermajority s2 c2 ∧
      ∀ (nc : node), (ctx.member nc s1 ∨ ctx.member nc s2) → precommitted_node nc V p1_fixed p2_fixed I)

-- QC Uniqueness: at most one interaction gets a prevote QC per view
invariant [unique_qc_prevote]
  (quorum_prevoted V p1_fixed p2_fixed I1 ∧ quorum_prevoted V p1_fixed p2_fixed I2) → I1 = I2

-- QC Uniqueness: at most one interaction gets a precommit QC per view
invariant [unique_qc_precommit]
  (quorum_precommitted V p1_fixed p2_fixed I1 ∧ quorum_precommitted V p1_fixed p2_fixed I2) → I1 = I2

-- QC Chain: precommit QC implies prevote QC (precommit comes after prevote in pipeline)
invariant [qc_precommit_implies_qc_prevote]
  quorum_precommitted V p1_fixed p2_fixed I → quorum_prevoted V p1_fixed p2_fixed I

-- QC Structural: QC implies an operator set it
invariant [qc_prevote_implies_operator]
  quorum_prevoted V p1_fixed p2_fixed I →
    ∃ (op : node), operator op V p1_fixed p2_fixed ∧ prevoted_operator op V p1_fixed p2_fixed I

invariant [qc_precommit_implies_operator]
  quorum_precommitted V p1_fixed p2_fixed I →
    ∃ (op : node), operator op V p1_fixed p2_fixed ∧ precommitted_operator op V p1_fixed p2_fixed I

-- QC Structural: no QC at view zero
invariant [no_qc_prevote_at_zero]
  ¬ quorum_prevoted tot_view.zero p1_fixed p2_fixed I

invariant [no_qc_precommit_at_zero]
  ¬ quorum_precommitted tot_view.zero p1_fixed p2_fixed I

-- QC Structural: QC not for genesis
invariant [qc_prevote_not_genesis]
  quorum_prevoted V p1_fixed p2_fixed I → I ≠ genesis

invariant [qc_precommit_not_genesis]
  quorum_precommitted V p1_fixed p2_fixed I → I ≠ genesis

-- QC Structural: QC interaction is valid
invariant [qc_prevote_is_interaction]
  quorum_prevoted V p1_fixed p2_fixed I → interactions I p1_fixed p2_fixed

invariant [qc_precommit_is_interaction]
  quorum_precommitted V p1_fixed p2_fixed I → interactions I p1_fixed p2_fixed

-- QC Node Chain: precommitted_node (set by respond_prevote) implies QC prevoted
invariant [precommitted_node_implies_qc_prevoted]
  (¬ ctx.is_byz N ∧ precommitted_node N V p1_fixed p2_fixed I) → quorum_prevoted V p1_fixed p2_fixed I
...
-- Every node has exactly one committed_ixn (functional uniqueness)
invariant [unique_committed_ixn]
  (committed_ixn N I1 ∧ committed_ixn N I2) → I1 = I2

-- Every node always has some committed_ixn (totality)
...
-- committed_ixn height is the max decided height
invariant [committed_ixn_height_upper_bound]
  (¬ ctx.is_byz N ∧ decided N V p1_fixed p2_fixed I ∧ committed_ixn N CI) → height I ≤ height CI
