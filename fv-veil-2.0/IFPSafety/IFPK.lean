-- IFPK: IFP model based on IFPJ with greater_than_third (f+1) support.
-- Uses Fix 1 approach: greater_than_third is an immutable relation with
-- assumptions, rather than a type class field. This avoids the #gen_spec
-- compiler IR error caused by enlarging the IFPByzQuorum type class.

import Veil

veil module IFPProtocolK
...
-- greater_than_third as an immutable relation (f+1 nodes of a context)
immutable relation ifp_greater_than_third : nodeset → nodeset → Bool
...
-- "Node N has a lock for participant P on an interaction of height H at view V"
-- i.e., ∃ I S, locked N P I S V ∧ height I = H
-- Analogous to HotStuff's votedh(N, H).
relation locked_at_height : node → participant → Nat → view → Bool

-- "Node N has a lock for participant P on a descendant of A at height H at view V"
-- i.e., ∃ I S, locked N P I S V ∧ ancestor A I ∧ height I = H
-- Analogous to HotStuff's votedb(N, B, H).
relation locked_descendant_at_height : node → participant → interaction → Nat → view → Bool

-- "The proposed interaction at view V has height H"
-- i.e., ∃ op I, proposed op V p1_fixed p2_fixed I ∧ height I = H
relation proposed_at_height : view → Nat → Bool
...
-- greater_than_third assumptions (f+1 axioms)

-- Membership containment: members of an f+1 set belong to the context
assumption ∀ (s c : nodeset) (n : node),
  ifp_greater_than_third s c → ctx.member n s → ctx.member n c

-- Any f+1 set contains at least one honest node
assumption ∀ (s c : nodeset),
  ifp_greater_than_third s c → (∃ (n : node), ctx.member n s ∧ ¬ ctx.is_byz n)

-- Monotonicity: a supermajority (2f+1) is also > 1/3 (f+1)
assumption ∀ (s c : nodeset),
  ctx.supermajority s c → ifp_greater_than_third s c

-- A supermajority and an f+1 set of the same context must share at least one node
-- (since 2f+1 + f+1 > 3f+1)
assumption ∀ (s1 s2 c : nodeset),
  (ctx.supermajority s1 c ∧ ifp_greater_than_third s2 c) →
    (∃ (n : node), ctx.member n s1 ∧ ctx.member n s2)

-- A supermajority contains an f+1 honest subset
-- (the ≥ f+1 honest members of any 2f+1 set form a greater_than_third set)
assumption ∀ (s c : nodeset),
  ctx.supermajority s c →
    (∃ (s' : nodeset), ifp_greater_than_third s' c ∧
      (∀ (n : node), (ctx.member n s' → (ctx.member n s ∧ ¬ ctx.is_byz n))))
...
after_init {
  ...
  -- Ghost init (height-aware)
  locked_at_height N P H V := decide $ (H = 0 ∧ V = tot_view.zero ∧ (P = p1_fixed ∨ P = p2_fixed));
  locked_descendant_at_height N P A H V := decide $ (A = genesis ∧ H = 0 ∧ V = tot_view.zero ∧ (P = p1_fixed ∨ P = p2_fixed));
  proposed_at_height V H := false;
}
...
action propose_repropose (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) {
  ...
  -- GHOST: proposed interaction height
  proposed_at_height v H := decide $ (proposed_at_height v H ∨ H = height ixn_max_1);
}
...
action propose_extend (op : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn_propose : interaction) {
  ...
  -- GHOST: proposed interaction height (parallel-safe: height of ixn_propose = height ixn_max_1 + 1)
  proposed_at_height v H := decide $ (proposed_at_height v H ∨ H = height ixn_max_1 + 1);
}
...
action respond_prevote (n : node) (v : view) (p1 p2 : participant) (c1 c2 : nodeset) (ixn : interaction) {
  ...
  if ( ctx.member n c1 ∧ ¬ ∃ (u : view), (tot_view.le u v ∧ locked n p1 ixn precommit u) ) then
    ...
    locked_at_height n p1 H v := decide $ (locked_at_height n p1 H v ∨ H = height ixn);
    locked_descendant_at_height n p1 A H v := decide $ (locked_descendant_at_height n p1 A H v ∨ (ancestor A ixn ∧ H = height ixn));
  if ( ctx.member n c2 ∧ ¬ ∃ (u : view), (tot_view.le u v ∧ locked n p2 ixn precommit u) ) then
    ...
    locked_at_height n p2 H v := decide $ (locked_at_height n p2 H v ∨ H = height ixn);
    locked_descendant_at_height n p2 A H v := decide $ (locked_descendant_at_height n p2 A H v ∨ (ancestor A ixn ∧ H = height ixn));
...
-- nodes from each context has prevote-locked. Uses ifp_greater_than_third instead of
  ...
  ∀ (v : view) (ixn : interaction) (n : node),
    ¬ ctx.is_byz n → ( (decided n v p1_fixed p2_fixed ixn ∧ ixn ≠ genesis) →
      (∃ (c1 c2 s1 s2 : nodeset), (participant_context p1_fixed c1 ∧ participant_context p2_fixed c2
        ...
        ∧ ifp_greater_than_third s1 c1 ∧ ifp_greater_than_third s2 c2
        ∧ ∀ (nc : node), (
...
-- ####################################################################
-- # Ghost Consistency Invariants (height-aware)
-- ####################################################################

-- locked_at_height forward: lock implies height ghost
invariant [locked_at_height_fwd]
  (locked N P I S V ∧ (P = p1_fixed ∨ P = p2_fixed)) → locked_at_height N P (height I) V

-- locked_at_height backward (honest only): ghost implies a real lock at that height
invariant [locked_at_height_bwd]
  (¬ ctx.is_byz N ∧ locked_at_height N P H V ∧ (P = p1_fixed ∨ P = p2_fixed)) →
    ∃ (I : interaction) (S : stage), locked N P I S V ∧ height I = H

-- locked_descendant_at_height forward
invariant [locked_descendant_at_height_fwd]
  (locked N P I S V ∧ ancestor A I ∧ (P = p1_fixed ∨ P = p2_fixed)) →
    locked_descendant_at_height N P A (height I) V

-- locked_descendant_at_height backward (honest only)
invariant [locked_descendant_at_height_bwd]
  (¬ ctx.is_byz N ∧ locked_descendant_at_height N P A H V ∧ (P = p1_fixed ∨ P = p2_fixed)) →
    ∃ (I : interaction) (S : stage), locked N P I S V ∧ ancestor A I ∧ height I = H

-- proposed_at_height forward
invariant [proposed_at_height_fwd]
  (proposed OP V p1_fixed p2_fixed I) → proposed_at_height V (height I)

-- proposed_at_height backward
invariant [proposed_at_height_bwd]
  proposed_at_height V H →
    ∃ (OP : node) (I : interaction), proposed OP V p1_fixed p2_fixed I ∧ height I = H

-- ####################################################################
-- # Height Monotonicity Invariants
-- ####################################################################

-- Height monotonicity of locks: locks at later views have equal or greater height
invariant [lock_height_monotone]
  (¬ ctx.is_byz N ∧ locked_at_height N P H1 V ∧ locked_at_height N P H2 U ∧
   (P = p1_fixed ∨ P = p2_fixed) ∧ tot_view.le V U) → H1 ≤ H2

-- Descendant lock height bound: locked descendant height ≥ ancestor height
invariant [locked_descendant_height_bound]
  (¬ ctx.is_byz N ∧ locked_descendant_at_height N P A H V ∧ (P = p1_fixed ∨ P = p2_fixed))
  → height A ≤ H

-- Combined: later lock height ≥ earlier descendant lock height
invariant [locked_descendant_height_monotone]
  (¬ ctx.is_byz N ∧ locked_descendant_at_height N P A H1 V ∧ locked_at_height N P H2 U ∧
   (P = p1_fixed ∨ P = p2_fixed) ∧ tot_view.le V U) → H1 ≤ H2

-- Unique height per lock view: an honest node has exactly one lock height per (P, V)
invariant [unique_lock_height]
  (¬ ctx.is_byz N ∧ locked_at_height N P H1 V ∧ locked_at_height N P H2 V ∧
   (P = p1_fixed ∨ P = p2_fixed)) → H1 = H2

-- Unique proposed height per view
invariant [unique_proposed_height]
  (proposed_at_height V H1 ∧ proposed_at_height V H2) → H1 = H2
