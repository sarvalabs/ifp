...
-- Sent lock entries can only exist for views the node has reached (current or past)
-- respond_prepare sets sent_lock at cur_view; set_view only advances cur_view
