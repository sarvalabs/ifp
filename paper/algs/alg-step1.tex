...
%\EndFor
%\item[] 
\LComment{\textbf{\color{black}As an operator $v$:}}
\For{$\m{view}.\m{begin}$}
	\If{$\m{v} = o_I$ for interaction $I$}
		\State $\m{I}_v \gets \m{I}$
		\State \textbf{multicast} \msg[]{\txt{Prepare}, I_v} to $\msc{ctx}(\m{I}_v)$
	\EndIf
\EndFor
\item[]	
...
\For{$\m{prepareTimeout}$}
    ...
    \State \Send \msg[]{\txt{Prepare}, \m{lockedQC_v^p}} to $\m{op}$
