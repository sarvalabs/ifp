...
\begin{lemma}\label{lem:conflictimpliesdiffslot}
    If there are two valid $\m{commitQCs}$ for two conflicting tesseracts $T$ and $T'$ then $\msc{view}(T)\neq \msc{view}(T')$.
\end{lemma}
\begin{proof}
    We prove by contradiction. 
    Suppose, there exist two QCs $\m{commitQC}$ and $\m{commitQC}'$ for the tesseracts $T$ and $T'$ formed during the same view.
    ...
    However, honest validators do not vote for multiple tesseracts involving the same participant. 
    Hence, the result follows by contradiction.
\end{proof}


\begin{lemma}\label{lem:highestqc} 
    If an honest validator has received $\m{precommitQC}$ for tesseract $T$ then the corresponding $\m{prevoteQC}$ will be a $lockedQC$ part of the \txt{Prepare} messages obtained by the operator in the next consensus instance for each participant $p\in P_T$.
\end{lemma} 
\begin{proof}
    If an honest validator received a $\m{precommitQC}$, then the validators whose \txt{PreCommit} messages comprise the quorum certificate have received $\m{prevoteQCs}$. % was received by an honest validator in the view, a quorum of validators from its ICS had sent \txt{PreCommit} messages. 
    Therefore, a quorum of validators are locked onto the $\m{prevoteQC}$. 
    ...
    Note that the quorum of validators locked onto the $\m{prevoteQC}$ consists of a supermajority of votes from $p$'s context.
    ...
    This validator sends the $\m{prevoteQC}$ as its locked QC in the \txt{Prepare} message and is therefore obtained by the operator. 
\end{proof}
...
\end{theorem}
\begin{proof}
    An honest validator commits a tesseract only upon receiving a valid $\m{precommitQC}$. 
    From \Cref{lem:conflictimpliesdiffslot}, it follows that the conflicting tesseracts' $\m{precommitQC}$s were formed in different views. 
    ...
    However, by \Cref{lem:highestqc}, we know that the operator for the subsequent view with an interaction for participant $p$ after $\msc{view}(T)$ will have the $\m{prevoteQC}$ formed for tesseract $T$ as part of its $\m{prepareQC}$
    The operator, while determining which interaction to propose, thus considers the above $\m{prevoteQC}$ as well. 
    ...
    % From \Cref{lem:highestqc}, we know that the $\m{prepareQC}$ for the tesseract $T'$ contains at least one $\m{prevoteQC}$ for the previously committed tesseract $T$. 
    % This $\m{prevoteQC}$ would have been obtained by the operator from the participant $p$'s context.
    % In the procedure for determining the interaction to propose, either the $\m{prevoteQC}$ would be filtered or not. 
    % If the $\m{prevoteQC}$ is not filtered, then the proposal has to be a $p$-child to tesseract $T$. 
    % However, if the $\m{prevoteQC}$ is filtered, then the proposal has to extend the latest state of each of the interacting participants, and therefore it will be a $p$-child of the committed tesseract $T$. 
    % Thus in both cases the tesseract $T'$ is a $p$-child of $T$ and hence cannot be conflicting with $T$. 
    % Note that operators cannot forge a $\m{prepareQC}$ for $T'$ since validators verify the quorum certificates and digital signatures are assumed to be ideal. 
    % Further, honest validators send a \txt{PreVote} only if the proposed tesseract is valid based on the proposal procedure undertaken by the operator with $\m{prepareQC}$.
    % Hence, the result follows.
\end{proof}

% \begin{theorem}
%     Two conflicting tesseracts cannot be committed by honest validators.
% \end{theorem}
% \begin{proof}
%     Case 1: The validators are the same. Follows from above lemma.
%     Case 2: Validators are different. By \Cref{lem:conflictimpliesdiffslot}, T was committed in a view earlier than T'. Thus, in a view between v and v', during the next active view, By \Cref{lem:highestqc}, the proposal has to extend T. Therefore, the view in which T' would have been committed, has to extend the proposal that extended T, if it was present. However, this implies that T' is a descendant of T. Thus, no conflicts. 

%     Just make this the above lemma. 
% \end{proof}

%Thus, Krama satisfies safety given that at least one set of validators in the ICS have strictly less than one-third of its validators Byzantine. This is achieved with high probability as given in \Cref{subsec:probanalysis}.
...
\begin{proof}
    ...
    Since the operators are honest, all validators in a participant's context receive the same set of \txt{Prepare} messages as operators broadcast them to the entire context. 
...
\input{algs/alg-outdated}

Additionally, during the \txt{Prepare} stage for a given participant set $P$, Byzantine validators can send outdated \txt{Vote} locks in whose place other tesseracts have been committed in participants not in $P$ to prevent the operator from making a proposal due to the presence of more than one \txt{Vote} locked tesseract. 
